...
int listarvendas(Prod produtos[], int *qtdv, Vendeu caixa[])
{
    int i, cont = 0, v=0;
    ...
    for(i=0; i< *qtdv;i++)
    {
        v=caixa[i].valorvenda/caixa[i].produto_vendido.preco;
        printf("Venda %d:\n", i + 1);
        printf("\tProduto: %s\n", caixa[i].produto_vendido.nome);
        printf("\tCódigo do Produto: %d\n", caixa[i].produto_vendido.codprod);
...
int venderproduto(Prod produtos[], int *qtdr, Vendeu caixa[], int *qtdv)
{
    system("cls");

    int i, ler = 0, repetir = 0;
    ...
    do
    {
        ...
        caixa[*qtdv].produto_vendido = produtos[ler];

        do
        {
            printf("=>quantas unidades de %s você quer vender? (%i) disponíveis\n\t=>aqui: ", produtos[ler].nome, produtos[ler].estoque);
            scanf("%d", &caixa[ler].unid);
        } while(caixa[ler].unid > produtos[ler].estoque);

        caixa[*qtdv].valorvenda = (caixa[ler].unid)*(produtos[ler].preco);

        printf("=>digite o código da venda n° %i\n\t=>aqui: ",(*qtdv) + 1);
        scanf("%i", &caixa[*qtdv].codvenda);

        (*qtdv)++;

        produtos[ler].estoque -= caixa[ler].unid;
...
int main()
{
    ...
    do
    {
        ...
        case 2:
            venderproduto(produtos, &qtdr, caixa, &qtdv);
            break;

        case 3:
            relatorios(produtos, qtdr, qtdv, caixa);
            break;
